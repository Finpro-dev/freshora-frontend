...
async function AppNavbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 h-15 sm:h-18 w-full flex gap-8 lg:gap-10 justify-between items-center px-5 sm:px-10 py-2 bg-brand-mist-200 shadow-md shadow-brand-mist-300/30">
      <AppLogo />
      <div className="flex sm:gap-5 w-full sm:w-[80%] justify-end">
        <NavigationMenu />
        <AppSearchBar />
      </div>
      {isLogin ? (
