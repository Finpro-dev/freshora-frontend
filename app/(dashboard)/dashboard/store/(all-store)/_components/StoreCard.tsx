...
function StoreCard({ store }: StoreCardProps) {
  ...
  return (
    <div
      ...
      className="cursor-pointer group w-full flex flex-col sm:flex-row gap-6 p-5 rounded-md border border-brand-mist-100 bg-brand-mist-100/10  shadow-sm shadow-brand-mist-300/60 transition-all duration-200 hover:shadow-md hover:border-brand-mist-200">
      {/* store-avatar */}
        ...
        <div className="mt-4 pt-4 border-t border-brand-mist-100 flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm font-semibold text-brand-mist-600 truncate">
              {store?.user
                ? `${capitalize(store.user?.firstName)} ${capitalize(store.user?.lastName)}`
                : `Assign required`}
            </p>
          </div>
        </div>
